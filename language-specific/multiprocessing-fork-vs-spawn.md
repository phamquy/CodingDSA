# Why Python Multiprocessing Needs Pickling on macOS (Fork vs. Spawn)

When you hit a `TypeError: cannot pickle 'mmap.mmap' object` on macOS, it can be confusing. Why does a child process need to "pickle" something that's supposed to be shared memory? The answer lies in how different operating systems create new processes.
