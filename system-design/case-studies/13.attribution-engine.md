...
- Ad events: impression logs, click logs (with identifiers like impression_id, click_id, request_id, campaign/ad/creative IDs, timestamps, cost, device/user IDs).
...
## Outputs: attributed conversion records and rollups used for:
  ...
  Pacing/optimization (ROI/CPA signals)
...
- Join conversions to eligible exposures using a configurable hierarchy:
  - deterministic IDs first (click_id / impression_id)
...
- `event_id`: unique ingestion id for dedupe (can equal impression_id/click_id/conv_id when available)
...
- Deterministic IDs (click_id) are cheaper, more accurate, and easier to audit.
...
- Clicks stream: maps click_id to impression_id and adds click metadata
...
- **versioned** (rule_version attached to outputs)
...
- **Deterministic join first** (click_id/impression_id), identity join only as fallback and policy-gated.
