...
- But hotspots can occur (e.g., huge publishers). We mitigate with:
  - composite keys (publisher_id + impression_id)
...
- Identity resolution outputs (user_key mapping)
...
- Cluster/sort by high-cardinality keys used in queries (campaign_id, advertiser_id).
...
- `identity_signal` events: `(id_type_a, id_value_a) linked_to (id_type_b, id_value_b)`
  - examples: cookie ↔ hashed_email, device_id ↔ login_id
...
3) **Missing join IDs** (no click_id on conversion)
	- Mitigation: fallback join by user_key + time window; label as lower confidence.
