#### **Design #2: Real-time Bidding System (RTB)**
...
Notes:
- Many exchanges send `tmax` (e.g., 80–120ms). We must enforce our own internal deadline, typically **min(100ms, tmax - safety_margin)**.
...
**(B) Fast-changing counters/state** (budgets, frequency)
Options:
