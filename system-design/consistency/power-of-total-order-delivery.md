...
*Definition:* Messages may be lost, but are never redelivered. If a failure occurs, the message is dropped.
*Motto:* "Better to lose it than to duplicate it."
...
The consumer reads a batch of messages, updates its "read pointer" (offset) to mark them as done, and *then* starts processing the business logic.
*   **Mechanism:**
...
*Definition:* Messages are never lost, but may be redelivered. Duplicate processing is possible.
*Motto:* "Better to duplicate it than to lose it."
...
*Definition:* Each message is processed effectively once. Neither lost nor duplicated in the final state.
*Note:* In distributed systems, this is technically "Exactly-Once *Processing* semantics." You can't prevent the network from sending a packet twice, but you can prevent the state from changing twice.
...
To prevent duplicates at the *ingestion* level (Writer -> Log):
*   **Mechanism:**
