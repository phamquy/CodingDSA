# Power-of-Two Choices Load Balancing: Deep Dive
...
**Key Paper**: "The Power of Two Choices in Randomized Load Balancing" (Azar, Broder, Karlin, Upfal, 1999)
...
- **d-choice hashing** ($d > 2$): Maximum load drops to $\log(\log n) / \log d$, but gains diminish rapidly
- **Weighted servers**: Algorithm adapts to heterogeneous capacity with minor modifications
- **Dynamic arrival/departure**: Maintains balance even with continuous churn
- **Non-uniform load**: Robust even when request patterns are skewed
...
Consistent hashing maps hash(user_id) → specific server:
...
- Adding 200 pods = rebuilding ring with $200 \times \text{virtual\_nodes}$ entries
...
Don't use it for:
- Bidder fleet (needs warm feature cache per campaign)
